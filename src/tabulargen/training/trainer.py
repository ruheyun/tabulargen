...
from tabulargen.models.math import update_ema
from tabulargen.data.dataset import TabularDataset
from tabulargen.artifacts import encoded_fingerprints
...
def train(
    ...
    checkpoint_path = Path(checkpoint_path or exp_path)
    log_path = Path(log_path or exp_path)
    if (checkpoint_path / "checkpoint.pt").exists():
        raise FileExistsError("Training run already exists; choose a new experiment.path")
    delu.random.seed(seed)
    encoding_hashes = encoded_fingerprints(exp_path)
    ...
    torch.save(diffusion._denoise_fn.state_dict(), checkpoint_path / 'model.pt')
    torch.save(ema_model.state_dict(), checkpoint_path / 'model_ema.pt')

    # The relative encoding path keeps the experiment directory movable as a unit.
    torch.save({
        'format_version': 2,
        ...
        'encoding': {'path': os.path.relpath(Path(exp_path).resolve(), checkpoint_path.resolve()), 'sha256': encoding_hashes},
        'train_params': {
            'epochs': epochs, 'lr': lr, 'weight_decay': weight_decay,
            'batch_size': batch_size, 'seed': seed, 'num_workers': num_workers, 'dp_params': deepcopy(dp_params),
        },
    }, checkpoint_path / 'checkpoint.pt')

    loss_history.to_csv(log_path / 'loss.csv', index=False)
